...
test("capacidades avanzadas de Familiar no crean segundo lanzador",async()=>{const guards=await readFile(resolve(root,"scripts/rules/familiar-guards.mjs"),"utf8");const familiar=await readFile(resolve(root,"templates/actor/familiar-sheet.hbs"),"utf8");const model=JSON.parse(await readFile(resolve(root,"template.json"),"utf8"));assert.equal(model.Actor.familiar.familiar.bondLevel,1);assert.equal(guards.includes("ActorClass.prototype.useFamiliarSense = async function"),true);assert.equal(guards.includes('"system.turn.action": false'),true);assert.equal(guards.includes("ActorClass.prototype.castFromFamiliar = async function"),true);assert.equal(guards.includes("const result = await this.useSpell(spell, { remoteOrigin: familiar })"),true);assert.equal(familiar.includes("Origen Remoto"),true);});
